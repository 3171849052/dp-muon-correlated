"""State-preserving switch from Optax AdamW to frozen-p AdamW."""
...
def p_star_from_optax(optimizer_state: Any, *, beta2: float, eps: float) -> PyTree:
  """Return ``1 / (sqrt(nu / (1-beta2**count)) + eps)`` from real state."""
...
class FrozenPAdamWState:
  """Adam count/momentum continued from warmup; ``nu`` and ``p_star`` freeze."""
...
def freeze_optax_adamw(optimizer_state: Any, *, beta2: float, eps: float) -> FrozenPAdamWState:
  """Switch after step tau without resetting count, first moment, or second moment."""
  adam = _adam_state(optimizer_state)
  return FrozenPAdamWState(
      count=adam.count, mu=adam.mu, frozen_nu=adam.nu,
...
class FrozenPAdamW:
  ...
  def update(self, gradients: PyTree, state: FrozenPAdamWState, params: PyTree):
    """Apply the exact Phase-II recurrence using the global Adam count."""
    ...
    updates = jax.tree_util.tree_map(
        ...
        mu, state.p_star, params)
    return updates, FrozenPAdamWState(count, mu, state.frozen_nu, state.p_star)


__all__ = [
    "FrozenPAdamW", "FrozenPAdamWState", "freeze_optax_adamw",
    "p_star_from_optax",
]
