"""YAML orchestration for the non-amplified IID DP-AdamW baseline."""
...
from .cifar10_driver import Cifar10DPAdamWTrainConfig, train_cifar10_dpadamw
...
class Cifar10DPAdamWExperimentConfig:
  algorithm: Literal["dpadamw"]
  name: str
  seed: int
  gpu: int
  data_dir: str
...
def load_cifar10_dpadamw_config(
    path: str | Path,
) -> Cifar10DPAdamWExperimentConfig:
  """Loads the strategy-free YAML schema for IID DP-AdamW."""
  source = Path(path)
  try:
  ...
  expected_sections = {
      "algorithm", "experiment", "runtime", "data", "model", "training", "adamw", "schedule", "privacy", "output"
  }
  ...
  if algorithm != "dpadamw":
    raise ValueError("DP-AdamW config requires algorithm: dpadamw")
  ...
  beta1 = _finite_float(adamw["beta1"], "adamw.beta1")
  if not 0.0 <= beta1 < 1.0:
    raise ValueError("adamw.beta1 must be in [0, 1)")
  beta2 = _finite_float(adamw["beta2"], "adamw.beta2")
  if not 0.0 <= beta2 < 1.0:
    raise ValueError("adamw.beta2 must be in [0, 1)")
  config = Cifar10DPAdamWExperimentConfig(
      ...
      learning_rate=_finite_float(adamw["learning_rate"], "adamw.learning_rate", positive=True),
      beta1=beta1,
      beta2=beta2,
      eps=_finite_float(adamw["eps"], "adamw.eps", positive=True),
      weight_decay=_finite_float(adamw["weight_decay"], "adamw.weight_decay", nonnegative=True),
...
def resolve_output_log_dir(config_path: str | Path) -> Path:
  log_dir = Path(load_cifar10_dpadamw_config(config_path).log_dir)
...
def prepare_cifar10_dpadamw_run(config_path: str | Path):
...
def run_cifar10_dpadamw(
    config_path: str | Path,
    *,
    resume_checkpoint: str | Path | None = None,
    run_dir: str | Path | None = None,
):
  """Derives the fixed-cycle privacy bound and runs IID DP-AdamW."""
  config = load_cifar10_dpadamw_config(config_path)
  source_yaml = Path(config_path).read_text(encoding="utf-8")
  document = yaml.safe_load(source_yaml)
  if not isinstance(document, Mapping):
  ...
  train_config = Cifar10DPAdamWTrainConfig(
  ...
  if resume_checkpoint is None:
    resolved = {
        ...
        "strategy": {"algorithm": "nonamplified_iid_dpadamw"},
