"""YAML orchestration for naive BandInvMF-correlated DP-AdamW.

This is a naive prefix-sum baseline.  BandInvMF only generates the correlated
private gradient; AdamW is a pure post-processing step.  No AdamW-aware
workload is used.
"""
...
class Cifar10BandInvDPAdamWExperimentConfig:
  """YAML-facing fields for ``Cifar10BandInvDPAdamWTrainConfig`` plus runtime."""

  algorithm: Literal["dp-adamw-correlated-naive"]
  name: str
  pretrained: str
  data_dir: str
  epochs: int
...
def load_cifar10_bandinv_dpadamw_config(
    path: str | Path,
) -> Cifar10BandInvDPAdamWExperimentConfig:
  """Loads the fixed-schema naive correlated DP-AdamW YAML configuration."""
  source = Path(path)
  try:
  ...
  if algorithm != BANDINV_DPADAMW_ALGORITHM:
    raise ValueError(
        f"naive correlated DP-AdamW config requires algorithm: {BANDINV_DPADAMW_ALGORITHM}"
    )
...
def resolve_output_log_dir(config_path: str | Path) -> Path:
  ...
  directory = Path(load_cifar10_bandinv_dpadamw_config(config_path).log_dir)
...
def prepare_cifar10_bandinv_dpadamw_run(config_path: str | Path):
...
def _train_config(
  ...
  return Cifar10BandInvDPAdamWTrainConfig(
...
def run_cifar10_bandinv_dpadamw(
    config_path: str | Path,
    *,
    resume_checkpoint: str | Path | None = None,
    run_dir: str | Path | None = None,
):
  """Delegates all training and privacy work to the naive correlated DP-AdamW trainer."""
  config = load_cifar10_bandinv_dpadamw_config(config_path)
  source_yaml = Path(config_path).read_text(encoding="utf-8")
  document = yaml.safe_load(source_yaml)
  if not isinstance(document, Mapping):
  ...
  if resume_checkpoint is None:
    ...
    MetricsCSVWriter(paths.metrics)
  return train_cifar10_bandinv_dpadamw(
